...
def get_recommendation(ticker: str, user_query: str, years: Optional[List[int]] = None, last_n: int = 3, top_k: int = 5) -> Dict[str, Any]:
	...
	try:
		if llm_service is None:
			raise RuntimeError("LLM service unavailable — Ollama is not accessible at http://localhost:11434")
		...
		if isinstance(llm_out, dict):
			llm_out = llm_out.get("answer", str(llm_out))
		
		# If answer is still empty, use fallback
		if not llm_out or llm_out.strip() == "":
			logger.warning("LLM returned empty answer, using context snippets as fallback")
			snippets = "\n\n".join([c["text"][:500] for c in top_chunks[:3]])
			llm_out = f"(LLM returned empty response) Context snippets:\n{snippets}"
	except Exception as e:
